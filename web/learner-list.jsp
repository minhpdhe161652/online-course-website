...
<head>
    ...
    <title>Quản lý Learner | Learning Management System</title>
...
<body>
    <div class="header">
        ...
        <h1><i class="fas fa-user-graduate"></i> Quản lý Learner</h1>
    </div>
    ...
    <div class="sort-section">
        ...
        <a href="?type=learner&sort=name&order=${param.sort == 'name' && param.order == 'ASC' ? 'DESC' : 'ASC'}" 
        ...
        <a href="?type=learner&sort=email&order=${param.sort == 'email' && param.order == 'ASC' ? 'DESC' : 'ASC'}" 
        ...
        <a href="?type=learner&sort=username&order=${param.sort == 'username' && param.order == 'ASC' ? 'DESC' : 'ASC'}" 
    ...
    <script>
        function confirmDelete(userId) {
            if (confirm('Bạn có chắc chắn muốn vô hiệu hóa người dùng này không?')) {
                window.location.href = '${pageContext.request.contextPath}/admin/users?action=delete&id=' + userId + '&type=learner';
            }
        }
        
        function confirmReactivate(userId) {
            if (confirm('Bạn có chắc chắn muốn kích hoạt lại người dùng này không?')) {
                window.location.href = '${pageContext.request.contextPath}/admin/users?action=reactivate&id=' + userId + '&type=learner';
            }
        }
    </script>
</body>
